...
/// # Errors
///
/// Returns an error when rows contain unsupported nested values, mix incompatible
/// scalar types, or cannot be represented as an Arrow record batch.
        ...
        for (field, value) in &flattened {
            match fields.entry(field.clone()) {
                std::collections::btree_map::Entry::Vacant(entry) => {
                    entry.insert(JsonScalarType::from_value(value));
                }
                std::collections::btree_map::Entry::Occupied(mut entry) => {
                    merge_json_scalar_type(field, entry.get_mut(), value)?;
                }
            }
        }
...
fn merge_json_scalar_type(
    field: &str,
    data_type: &mut JsonScalarType,
    value: &Value,
) -> Result<(), ArrowFilterRowBridgeError> {
...
/// # Errors
///
/// Returns an error when a column type cannot be represented by the filter JSON
/// shape or enum descriptor metadata is missing or invalid.
...
/// # Errors
///
/// Returns an error when `row` is out of bounds, a column type cannot be
/// represented by the filter JSON shape, or enum descriptor metadata is invalid.
...
mod tests {
    ...
    #[test]
    fn arrow_filter_row_bridge_rejects_incompatible_scalar_types() {
        for (field, rows) in [
            ("flag", vec![json!({"flag": true}), json!({"flag": "true"})]),
            ("value", vec![json!({"value": 7}), json!({"value": "7"})]),
        ] {
            let error = json_rows_to_arrow_filter_batch(&rows)
                .expect_err("incompatible scalar types must not be stringified");

            assert_eq!(
                error,
                ArrowFilterRowBridgeError::MixedJsonTypes {
                    field: field.to_string(),
                }
            );
        }
